...
class Flow(torch.nn.Module, dist.Distribution):
    ...
    def sample(
        ...
        with torch.no_grad():
            ...
            x = self.bijector._forward(x, context)
            return x
    ...
    def rnormalize(
        ...
        return self.bijector._inverse(value, context)  # type: ignore
    ...
    def log_prob(
        ...
        # TODO: Input x=x if value is a bijective tensor
        x = self.bijector._inverse(y=value, context=context)  # type: ignore
        log_prob = -_sum_rightmost(
            self.bijector._log_abs_det_jacobian(x, value, context),  # type: ignore
            event_dim - self.bijector.domain.event_dim,  # type: ignore
        )
